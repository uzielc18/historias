...
class Form
{
     ...
     * @return string
     */
    public static function select($field, $data, $attrs = NULL, $value = NULL,$blank = 'Seleccione')
    {
        if(is_array($attrs)){
            $attrs = Tag::getAttrs($attrs);
        }
    ...
    public static function dbSelect($field, $show = NULL, $data = NULL, $blank = 'Seleccione', $attrs = NULL, $value = NULL)
    {
        ...
        if($data === NULL){
			...
			/* validar mas de un $show 478 y 495*/
			$c_s=count($SHOW=explode(',',$show));
        ...
        foreach($data as $p) {
			...
			/*mejorado por uziel 511-521 y 522*/
			if($c_s>0)
			{
				$p_s='';
				for($i=0;$i<$c_s;$i++)
				{
					if(($c_s-1)==$i)$p_s.=$p->$SHOW[$i]; else $p_s.=$p->$SHOW[$i].'-';
				}
			}else
			{
				$p_s=$p->$show;
			}
            $options .= '>' . htmlspecialchars($p_s, ENT_COMPAT, APP_CHARSET) . '</option>';
        }
